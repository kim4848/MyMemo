...
export default function DashboardPage() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="font-[family-name:var(--font-heading)] text-2xl font-semibold text-text-primary">Sessions</h1>
      </div>

      <TagFilter />

      {filteredSessions.length === 0 ? (
        <div className="rounded-xl border border-border bg-bg-card p-12 text-center shadow-sm">
          ...
          {sessions.length === 0 && (
            <Link
              to="/record"
              className="mt-4 inline-flex items-center gap-2 rounded-lg bg-accent px-4 py-2.5 font-medium text-white hover:bg-accent-hover transition-colors"
            >
              Start Recording
            </Link>
          )}
