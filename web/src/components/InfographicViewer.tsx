...
export default function InfographicViewer({ sessionId }: Props) {
  ...
  const handleDownloadPng = useCallback(() => {
    ...
    const mimeType = infographic.imageFormat === 'jpeg' ? 'image/jpeg' : 'image/png';
    const blob = new Blob([bytes], { type: mimeType });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `infographic-${sessionId}.${infographic.imageFormat}`;
    a.click();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="relative flex max-h-[90vh] w-full max-w-3xl flex-col rounded-2xl border border-navy-700 bg-navy-900 shadow-2xl">
            ...
            <div className="overflow-auto p-5">
              <img
                src={`data:image/${infographic.imageFormat};base64,${infographic.imageContent}`}
                alt="Infographic"
